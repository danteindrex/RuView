/**
 * RuView Observatory â€” Main Scene Orchestrator
 *
      ...
      fetch(`${base}/health`, { signal: AbortSignal.timeout(1500) })
        ...
        .then(data => {
          if (data && data.status === 'ok') {
            ...
            console.log('[Observatory] Sensing server detected at', base, 'â†’', wsUrl);
            this.settings.dataSource = 'ws';
  ...
  _connectWS(url) {
    ...
    try {
      ...
      this._ws.onclose = () => {
        ...
        this._ws = null;
        this._liveData = null;
        if (this.settings.dataSource === 'ws') this.settings.dataSource = 'ws';
        this._hud.updateSourceBadge('ws', null);
