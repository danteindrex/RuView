//! Wave three-metric acceptance test (ADR-031).
//!
...
impl std::fmt::Display for WaveTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            WaveTier::Fail => write!(f, "FAIL"),
            WaveTier::Bronze => write!(f, "BRONZE"),
            WaveTier::Silver => write!(f, "SILVER"),
            WaveTier::Gold => write!(f, "GOLD"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct WaveAcceptanceResult {
...
impl WaveAcceptanceResult {
    /// A human-readable summary of the acceptance test.
    pub fn summary(&self) -> String {
        format!(
            "Wave Tier={} | PCK={:.3} OKS={:.3} | MOTA={:.3} IDsw={} | Breathing={:.1}BPM err",
            self.tier,
...
mod tests {
    ...
    fn tier_determination_gold() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WaveTier::Gold);
    }
    ...
    fn tier_determination_silver() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WaveTier::Silver);
    }
    ...
    fn tier_determination_bronze() {
        ...
        let tr = TrackingResult { passes: true, ..Default::default() };
        let vs = VitalSignResult { passes: false, ..Default::default() };
        assert_eq!(determine_tier(&je, &tr, &vs), WaveTier::Bronze);
    }
    ...
    fn tier_determination_fail() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), WaveTier::Fail);
    }

    #[test]
    fn tier_ordering() {
        assert!(WaveTier::Gold > WaveTier::Silver);
        assert!(WaveTier::Silver > WaveTier::Bronze);
        assert!(WaveTier::Bronze > WaveTier::Fail);
    }
