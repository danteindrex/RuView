...
# the Wave desktop app driving this over SSH). Only run it on a real TTY.
if [ -t 0 ]; then
...
echo -e "  ${CYAN}bash nexmon_startup.sh${NC}"
echo ""
# Stable machine-readable marker so an automated driver (Wave desktop) can
# tell an intentional reboot apart from a mid-install SSH failure. Do not remove.
echo "WAVE_NEXMON_REBOOT"
