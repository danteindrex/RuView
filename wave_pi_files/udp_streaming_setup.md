...
## 3. Clone this repository onto the raspberry pi, as it contains the agent node responsible for converting pcap CSI packets to the ADR-018 format required by Wave

```bash
git clone https://github.com/danteindrex/Wave.git
```
...
```bash
cd ~
cp Wave/wave_pi_files/nexmon_bridge.py scripts/nexmon_bridge.py
...
docker run -p 3000:3000 -p 3001:3001 -p 5005:5005/udp -e CSI_SOURCE=esp32 --name wave ruvnet/wifi-densepose:latest
```
