...
export class HudController {
  ...
  initSettings() {
    ...
    // Live-only mode: force WebSocket and disable demo fallback selection.
    s.dataSource = 'ws';
    dsSel.value = 'ws';
    dsSel.disabled = true;
    dsSel.addEventListener('change', (e) => {
      ...
      this.updateSourceBadge('ws', obs._ws);
      this.saveSettings();
    });
    document.getElementById('ws-url-row').style.display = 'flex';
    ...
    document.getElementById('btn-export-settings').addEventListener('click', () => {
      ...
      a.download = 'wave-observatory-settings.json';
      a.click();
    });
  ...
  saveSettings() {
    try {
      localStorage.setItem('wave-observatory-settings', JSON.stringify(this._obs.settings));
    } catch {}
  }
