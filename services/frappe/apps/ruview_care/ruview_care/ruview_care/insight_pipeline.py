...
Called via frappe.enqueue("ruview_care.ruview_care.insight_pipeline.run_pipeline", session_name=...)
...
def _make_llm():
    import os
    api_key = os.getenv("OPENAI_API_KEY") or frappe.db.get_single_value("RuView Settings", "openai_api_key")
    if not api_key:
        return None
...
def run_pipeline(session_name: str) -> str:
    ...
    if session.deployment and r.get("risk_level") in ("high", "critical"):
        dep = frappe.get_doc("RuView Deployment", session.deployment)
